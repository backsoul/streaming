...
func (s *Server) Serve(l net.Listener) error {
	...
	mux.HandleFunc("/streaming", func(w http.ResponseWriter, r *http.Request) {
		// Lee el archivo HTML desde el sistema de archivos
		htmlFile, err := os.Open("protocol/api/statics/index.html")
		if err != nil {
			http.Error(w, "No se pudo abrir el archivo HTML", http.StatusInternalServerError)
			return
		}
		defer htmlFile.Close()

		// Establece el encabezado Content-Type apropiado para HTML
		w.Header().Set("Content-Type", "text/html")

		// Copia el contenido del archivo HTML en la respuesta HTTP
		_, err = io.Copy(w, htmlFile)
		if err != nil {
			http.Error(w, "Error al servir el archivo HTML", http.StatusInternalServerError)
			return
		}
	})
